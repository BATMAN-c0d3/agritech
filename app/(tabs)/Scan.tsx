      ...
      <View style={styles.header}>
        <SafeAreaView>
          ...
          <Text style={styles.headerSubtitle}>
            Scan or upload an image of your crop and we'll diagnose potential diseases
          </Text>
        </SafeAreaView>
      </View>
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        ...
        <View style={styles.tipsContainer}>
          ...
          <View style={styles.tipItem}>
            ...
            <Text style={styles.tip}>Ensure the crop is well-lit for a clear image.</Text>
          </View>
          <View style={styles.tipItem}>
            <Entypo name="crop" size={18} color="#34A853" />
            <Text style={styles.tip}>Focus on the affected area of the crop.</Text>
          </View>
          <View style={styles.tipItem}>
        ...
        <View style={styles.buttonContainer}>
          <TouchableOpacity style={styles.iconButton} onPress={captureImageFromCamera}>
        ...
        {predictions.length > 0 ? (
          <View style={styles.predictionsContainer}>
            ...
            {predictions.map((prediction, index) => {
              const roundedConfidence = Math.round(prediction.confidence_score * 100);
              return (
                <View key={index} style={styles.predictionItem}>
                  <Text style={styles.predictionName}>{prediction.name}</Text>
                  <View style={styles.confidenceContainer} className='flex-row justify-between items-center'>
                    <View className='flex-row items-center space-x-1'>
                      <Text className='text-[18px] font-semibold'>{prediction.class.split(' ')[1]}</Text>
                      <Text className='text-[18px] font-semibold'>{prediction.class.split(' ')[2]}</Text>
                    </View>
                  ...
                  <View style={styles.customProgressBar}>
                    <View
                      style={[
                        ...
                        { width: `${roundedConfidence}%`, backgroundColor: getConfidenceColor(roundedConfidence) },
                      ]}
                    />
                  </View>
                  {
                    ...
                    <View className='mt-5'>
                      <Text className='text-center font-semibold text-lg'>
                        {prediction.class == "4 Healthy plant" ? "Tips to maintain your plant health" : "Tips to figth the disease"}
                      </Text>
                      {prediction.tips.map((tip: any, index: number) => {
                        return (
                          <Text key={index} className='mt-3 font-medium'>{tip}</Text>
                        )
                      })
                      }
                    </View>
          ...
          <View style={styles.diseaseInfoContainer}>
            ...
            <Text style={styles.diseaseInfoText}>
              After scanning, we will provide a list of potential diseases affecting your crop along
